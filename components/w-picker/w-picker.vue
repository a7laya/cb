	...
	<view class="w-picker">
		...
		<view class="w-picker-cnt" :class="{'show':showPicker}">
			<view class="w-picker-hd" @touchmove.stop.prevent catchtouchmove="true">
			  <view class="w-picker-btn" @tap="pickerCancel">{{text_cancel}}</view>
			  <view class="w-picker-btn" :style="{'color':themeColor}" @tap="pickerConfirm">{{text_confirm}}</view>
			</view>
	...
	export default {
		...
		computed:{
			text_cancel(){
				if(!this.language)
				return '取消'
				return this.language['cancel'][this.languageType]
			},
			text_confirm(){
				if(!this.language)
				return '确定'
				return this.language['confirm'][this.languageType]
			},
		},
		props:{
			languageType: {
